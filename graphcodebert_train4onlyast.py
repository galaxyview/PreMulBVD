...
learning_rate = 1e-5
# 加载 .pth 文件
data = torch.load("datasets/pth/Juliet/CWE_graphastonly_mixed.pth")
checkpoint_path = 'checkpoints/finetune/mixed_batch64/CWE_mixed_epoch_13.pt'
...
class ASTDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return ast_encoding, torch.tensor(label)
    
class GraphCodeBERTFineTuner4ast(nn.Module):
    def __init__(self, graphcodebert):
        super(GraphCodeBERTFineTuner4ast, self).__init__()
    ...
    def forward(self, ast_input):
        ...
        # 通过分类器得到预测结果
        output = self.classifier(ast_outputs)
...
def validate():
    ...
    val_dataset = ASTDataset(eval_data, tokenizer)
    val_loader = DataLoader(val_dataset, batch_size=batch_size, shuffle=False)

    model = GraphCodeBERTFineTuner4ast(graphcodebert).to(device)
...
def train():
    ...
    # 创建训练集和评估集的数据集实例
    train_dataset = ASTDataset(train_data, tokenizer)
    eval_dataset = ASTDataset(eval_data, tokenizer)
    ...
    start_epoch = 0
    best_f1 = 0
    save_path = 'checkpoints/finetune/mixed_batch64/CWE_mixed_epoch_{epoch}.pt'
    ...
    for epoch in range(start_epoch, end_epoch):  # 设定训练的 epoch 数
        ...
        with tqdm(total=len(train_dataloader), desc=f"Epoch {epoch+1}/{end_epoch}") as pbar:
            for ast_input, label in train_dataloader:
                # 将数据移到当前 GPU 设备
                ast_input = {key: value.to(device) for key, value in ast_input.items()}
                label = label.to(device)
