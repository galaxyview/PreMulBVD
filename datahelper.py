...
class DataHelper:
    ...
    def do_query(self, db_path, optimization="O2", where="", limit=""):
        ...
        try:
            ...
            # sql = """select function_name, elf_file_name, elf_path, arch_name, caller, callee, ast_pick_dump from function """ + where + " " +limit
            sql = """select function_name, elf_file_name, elf_path, arch_name, ast_pick_dump from function"""
            l.info(sql)
    ...
    def loads_ast_tree(self, ast_pickle_dumps):
        ...
        return pickle.loads(ast_pickle_dumps)
...
# ============== ast  encode ======================
    ...
    def get_ast_hash_encode(self, db):
        try:
            db_conn = self.load_database(db)
            cur = db_conn.cursor()
            #sql = """select function_name, elf_path, pseudocode, ast_pick_dump, ast_hash_encode from function"""
            sql = """select function_name, elf_path, pseudocode, ast_pick_dump, address from function"""
            cur.execute(sql)
...
def mk_ast_hash_encode_dataset_fake():
    dh = DataHelper()
    binary_good_path = get_all_file_paths("/home/ubuntu/workspace/Asteria/datasets/C/testcases/CWE121_Stack_Based_Buffer_Overflow/s01/binary_good/sqlite")
    binary_bad_path = get_all_file_paths("/home/ubuntu/workspace/Asteria/datasets/C/testcases/CWE121_Stack_Based_Buffer_Overflow/s01/binary_bad/sqlite")
    dbs = binary_good_path + binary_bad_path
    # 正则表达式匹配规则
    pattern = re.compile(r'(\d+\.o\.sqlite|a+\.o\.sqlite)$')
    sigle_function_patterns = ['good']  # 单缺陷good函数命名规则
    multi_function_patterns = ['G2B','B2G']    # 多缺陷good函数命名规则
    function_patterns = sigle_function_patterns + multi_function_patterns
    # 筛选出符合条件的元素
    dbs = [item for item in dbs if pattern.search(item)]

    #dbs = ["/home/ubuntu/workspace/Asteria/data/openssl-x86/bin/openssl-x86.sqlite","/home/ubuntu/workspace/Asteria/data/openssl-x64/bin/openssl-x64.sqlite"]
    for db in dbs:
        dh.generate_ast_hash_encode(db)
    all_function_pairs =[] #[(ast1, ast2, label)]
    for idx in range(len(dbs)):
        s_db = dbs[idx]
        s_functions = list(dh.get_ast_hash_encode(s_db))
        for idx2 in range(idx, len(dbs)):
            t_db = dbs[idx2]
            t_functions = list(dh.get_ast_hash_encode(t_db))
            print("source db: %s , target db %s" %(s_db, t_db))
            if s_db.split('/')[-1].rpartition('_')[0] == t_db.split('/')[-1].rpartition('_')[0]:  # 筛选是否为同一函数
                if s_db in binary_good_path and t_db in binary_good_path:   # 挑选正样本
                    for s_func in s_functions:
                        for t_func in t_functions:
                            if (s_db == t_db and s_func[0] == t_func[0]) or s_func[0].endswith('_good') or t_func[0].endswith('_good'): # 排除s_func和t_func是同一个的情况，同时删除主函数
                                continue
                            if s_db.split('/')[-1].count('CWE') == 2:   # 区分单漏洞和多漏洞的函数名
                                for fp in multi_function_patterns:
                                    if fp in s_func[0] and fp in t_func[0]:
                                        all_function_pairs.append((s_func[:-1],
                                                t_func[:-1], 1))
                                        continue
                            elif s_db.split('/')[-1].count('CWE') == 1:
                                for fp in sigle_function_patterns:
                                    if fp in s_func[0] and fp in t_func[0]:
                                        all_function_pairs.append((s_func[:-1],
                                                t_func[:-1], 1))
                                        continue
                elif s_db in binary_good_path and t_db in binary_bad_path:   # 挑选负样本中good和bad的情况去除bad和bad的情况，(ast1,ast2)与(ast2,ast1)只计入一次负样本
                    for s_func in s_functions:
                        for t_func in t_functions:
                            for fp in function_patterns:
                                if fp in s_func[0]: # 去掉一些无意义函数，只保留good函数
                                    all_function_pairs.append((s_func[:-1],
                                               t_func[:-1], -1))
                                    continue
            '''
            ...
            for idx, s_func in enumerate(s_functions):
                ...
                while i < len(sims) and count>0:
                    if t_functions[sims[i][1]][0] != s_functions[0]: #
                        all_function_pairs.append((s_func[:-1], t_functions[sims[i][1]][:-1], -1))
                        count-=1
                    i+=1
            for key in same_name_functions:
                if len(same_name_functions[key]) == 2:
                    ...
                    t_idx = same_name_functions[key][0]
                    try:
                        all_function_pairs.append((s_functions[s_idx][:-1],
                                               t_functions[t_idx][:-1], 1))
                    except Exception as e:
                        print(e)
            '''
    import torch
    torch.save(all_function_pairs, "/home/ubuntu/workspace/Asteria/datasets/pth/juliet_datasets_CWE121.pth")
...
if __name__ == '__main__':
    ...
    mk_ast_hash_encode_dataset_fake()